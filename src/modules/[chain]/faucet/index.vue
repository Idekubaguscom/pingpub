...
<template>
    <div>
        <div class="flex flex-col items-center justify-center mb-6 mt-14 gap-4">
            <img v-if="chainStore.current?.logo" :src="`${chainStore.current?.logo}`" class="w-16 rounded-md-md" />
            <div v-else class="w-16 rounded-md-full">
        ...
        <div class="bg-base-100 my-5 px-4 pt-3 pb-4 rounded-md shadow">
            <h2 class="card-title">Get Tokens</h2>
            <input type="text" v-model="address" class="mt-4 mb-4 w-full border border-gray-300 rounded-md-md p-2"
                :class="{'input-error' : !validAddress}"
                :disabled="notReady" placeholder="Enter your address" />
            <button class="btn btn glass w-full bg-primary text-white" :disabled="notReady" @click="claim()">Get
                Tokens</button>
        </div>
        ...
        <div class="bg-base-100 my-5 px-4 pt-3 pb-4 rounded-md shadow">
            ...
            <div class="mt-4">
                ...
                <div class="mockup-code bg-base-200 my-2 gap-4">
                    ...
                    <pre></pre>
                    <a class=" btn-ghost text-white rounded-md-md p-2 ml-4"
