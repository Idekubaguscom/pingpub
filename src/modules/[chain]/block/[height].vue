...
<template>
  <div>
    <div v-if="isFutureBlock" class="text-center">
      <div v-if="remainingBlocks > 0">
        ...
        <div class="pt-10 flex justify-center">
          <table class="table w-max rounded-md primary border border-gray-400 dark:border-gray-400">
            <tbody>
              ...
              <tr v-if="edit">
                <td colspan="2" class="text-center">
                  ...
                  <div class="join">
                    ...
                    <button class="btn btn glass join-item" @click="updateTarget()">{{ $t('block.btn_update') }}</button>
                  </div>
                  </p>
                </td>
              </tr>
    ...
    <div v-else>
        ...
        <h2 class="card-title flex flex-row justify-between">
          ...
          <div class="flex" v-if="props.height">
            <RouterLink :to="`/${store.blockchain.chainName}/block/${height - 1}`"
              class="btn btn glass btn-sm p-1 text-2xl mr-2">
              <Icon icon="mdi-arrow-left" class="w-full h-full" />
            </RouterLink>
            <RouterLink :to="`/${store.blockchain.chainName}/block/${height + 1}`"
              class="btn btn glass btn-sm p-1 text-2xl">
      ...
      <div class="primary px-4 pt-3 pb-4 rounded-md mb-4 shadow border border-gray-400 dark:border-gray-400">
